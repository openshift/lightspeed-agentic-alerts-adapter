...
var _ = Describe("Deduplication", func() {
	Context("severity filtering", func() {
		...
		AfterEach(func() {
			...
			if err := framework.ResolveAlert(context.Background(), infoLabels); err != nil {
				errs = append(errs, fmt.Errorf("resolve info alert: %w", err))
			}
			if err := framework.ResolveAlert(context.Background(), noneLabels); err != nil {
				errs = append(errs, fmt.Errorf("resolve none alert: %w", err))
			}
			...
			if _, err := framework.DeleteResourcesByLabel(context.Background(),
				...
				cfg.Namespace,
				"agentic.openshift.io/alert-name=e2etestseverityinfo",
			); err != nil {
				errs = append(errs, fmt.Errorf("delete info AgenticRuns: %w", err))
			}
			if _, err := framework.DeleteResourcesByLabel(context.Background(),
				agenticRunAPIResource,
				cfg.Namespace,
				"agentic.openshift.io/alert-name=e2etestseveritynone",
			); err != nil {
				errs = append(errs, fmt.Errorf("delete none AgenticRuns: %w", err))
			}
		...
		It("should not create AgenticRun for info-severity alerts", func() {
			...
			err := framework.InjectAlert(context.Background(), framework.Alert{Labels: infoLabels})
			Expect(err).NotTo(HaveOccurred(), "failed to inject info-severity alert")
		...
		It("should not create AgenticRun for none-severity alerts", func() {
			...
			err := framework.InjectAlert(context.Background(), framework.Alert{Labels: noneLabels})
			Expect(err).NotTo(HaveOccurred(), "failed to inject none-severity alert")
