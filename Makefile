...
E2E_IMAGE_REF = openshift-lightspeed/$(BINARY_NAME):e2e
...
ifeq ($(GINKGO),)
	...
	$(eval GINKGO=$(shell gobin=$$(go env GOBIN); if [ -n "$$gobin" ]; then echo "$$gobin/ginkgo"; else echo "$$(go env GOPATH)/bin/ginkgo"; fi))
endif
...
undeploy-e2e:
	oc delete -f manifests/ --ignore-not-found -n openshift-lightspeed
